...
function parseDescription(html: string) {
  if (typeof window === "undefined")
    return { overview: html, features: [] as string[] };
...
function paintEdges(root: HTMLElement) {
  root
    .querySelectorAll(
      ".wf-sketch, .wf-photo, .wf-chip, .wf-btn, .wf-stack, .wf-xbox",
    )
    .forEach((el) => {
...
const CHECK_SVG = (
  <svg
    viewBox="0 0 24 24"
    aria-hidden="true"
    style={{ position: "absolute", left: 0, top: 10, width: 22, height: 22 }}
  >
    <path
...
export function WireframeProjectDetail({ project }: { project: Project }) {
  ...
  useEffect(() => {
    ...
    const handler = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowLeft")
        setLightboxIdx((p) =>
          p === null ? null : p === 0 ? images.length - 1 : p - 1,
        );
      if (e.key === "ArrowRight")
        setLightboxIdx((p) =>
          p === null ? null : p === images.length - 1 ? 0 : p + 1,
        );
    };
      ...
      <div style={{ maxWidth: 1080, margin: "0 auto", padding: "0 28px" }}>
        ...
        >
          ...
          <div className="wf-reveal d2" style={{ position: "relative" }}>
            ...
            <div
              className="wf-note"
              style={{ top: -26, right: -10, transform: "rotate(5deg)" }}
            >
              the real thing
            </div>
          </div>
        ...
        >
          <div className="wf-reveal">
            ...
            >
              <span
                style={{
                  color: "var(--wf-accent)",
                  fontFamily: "var(--wf-marker)",
                  fontSize: 14,
                }}
              >
                //
              </span>
              <h2 className="wf-h" style={{ fontSize: 28 }}>
                The idea
              </h2>
            </div>
            <div
            ...
            {features.length > 0 && (
              <>
                ...
                <ul
                  style={{ padding: 0, margin: "18px 0 0", listStyle: "none" }}
                >
                  {features.map((feat, i) => (
                    <li
                      key={i}
          ...
          {/* Built with */}
          <aside
            className="wf-sketch wf-stack wf-reveal d1"
            style={{ position: "relative" }}
          >
            <h4 className="wf-h" style={{ fontSize: 22, marginBottom: 14 }}>
              Built with
            </h4>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 9 }}>
              {project.technologies.map((tech) => (
                <span key={tech.name} className="wf-sketch wf-chip">
                  {tech.logoUrl && (
                    <img src={tech.logoUrl} alt="" className="wf-logo" />
                  )}
                  {tech.name}
                </span>
              ))}
            ...
            <div
              className="wf-note"
              style={{ bottom: -30, right: 10, transform: "rotate(-3deg)" }}
            >
              hover a chip,
              <br />
              it colors in ✦
