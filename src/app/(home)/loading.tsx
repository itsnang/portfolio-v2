    ...
    <main className="w-full max-w-4xl min-h-screen">
      ...
      <section className="pt-4 space-y-4">
        ...
        <div className="columns-2 gap-4 sm:columns-3">
          {[...Array(6)].map((_, idx) => (
            <Skeleton key={idx} className="mb-4 w-full rounded-lg aspect-3/4" />
          ))}
        </div>
      </section>
      <section className="py-6">
        ...
        <div className="flex pt-4 flex-wrap gap-3">
          {[...Array(7)].map((_, index) => (
            <div key={index} className="shrink-0 rounded-md px-3 py-2 shadow-xs ring-1 ring-inset ring-gray-300 dark:ring-gray-700 bg-white dark:bg-gray-900 inline-flex items-center gap-1">
      ...
      <section className="flex flex-col gap-y-3 py-6">
        ...
        <div className="grid w-full grid-cols-1 gap-3 sm:grid-cols-2">
          {Array.from({ length: 3 }).map((_, index) => (
            <Card key={index} className="flex w-full flex-col overflow-hidden border hover:shadow-lg transition-all duration-300 ease-out h-full">
              ...
              <CardFooter className="px-4 pb-4">
                <div className="flex flex-row flex-wrap items-start gap-1">
                  {[1, 2].map((i) => <Skeleton key={i} className="h-6 w-20" />)}
                </div>
              </CardFooter>
            </Card>
          ))}
